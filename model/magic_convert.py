...
def main():
    ...
    if (isfile):
        ...
        if (sys.argv[1] == "d2b"):
            ...
            print bcolors.OKBLUE + "Result: %s.%s" % (
            result[0:(n_bits - n_frac)], result[(n_bits - n_frac):n_bits]) + bcolors.ENDC
            print bcolors.OKGREEN + "Thanks for using converter!" + bcolors.ENDC
        else:
            result = convert2dec(value, n_bits, n_frac)
...
def convert2fixed_directly(n_dec,n_frac,value,forward_backward):
        n_bits = n_dec+n_frac
        n_frac = n_frac
        value = value
        if (forward_backward == "d2b"):
            value = float(value)
            result = convert2fixed(value, n_bits, n_frac)
            return "%s.%s" % ( result[0:(n_bits - n_frac)], result[(n_bits - n_frac):n_bits])
        else:
            result = convert2dec(value, n_bits, n_frac)
            return "%s" % (result)

if __name__ == "__main__":
    convert2fixed_directly(2, 2, '0110', 'b2d')
